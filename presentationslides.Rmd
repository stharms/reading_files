...
setwd("~/Documents/R/Rusergroup/reading_slides")#set working directory to wherever your file is located
...
---
# Reading from common file types
...
- If data isn't clean, you may need to read it in and clean it before you  convert to the data type you want
- The `type_convert()` function is like the `col_types` argument in `read_*` but is used after the data is read in
- Process:
  + Read in data as character vector
  + Cleaning/munging manually to remove extra characters / NAs / Invalid entries
...
- For general tab/comma/whitespace delimited files: 
  + `read.table`/`read_table`, specify the delmiter with `sep` argument
  + `read.delim`/`read_delim` and specify the delimiter with `delim` argument
  
- For fixed width files, `read.fwf`/`read_fwf` for fixed width files
  + This format can often be read faster than less-structured data
  
- For unstructured text files, use `readLines` or `read_lines`

- For extra large files, `read_csv_chunked` can read a file in chunks
  + Useful when you want to aggregate or filter the data without reading into memory
...
```{r, echo = TRUE}
#this is actually slower than read_csv() because the file is small
data.table::fread('18collegebasketball.csv') %>% 
  summary()
```
